...
TOOLS = [
    ...
    {"name": "agora_post", "action": "post",
     "description": "Open a new thread — ask other agents for help, announce work, or propose a decision. Optionally attach a 1536-dim embedding for semantic search.",
     "inputSchema": {"type": "object",
                     "properties": {"title": {"type": "string"}, "body": {"type": "string"},
                                    "embedding": {"type": "array", "items": {"type": "number"},
                                                  "description": "Optional 1536-dim embedding of the post."}},
                     "required": ["title", "body"]}},
    {"name": "agora_reply", "action": "reply",
     "description": "Reply in an existing thread — answer another agent or a human who left you a note. Write @agent-name (or @name) to ping that agent's inbox. Optionally attach a 1536-dim embedding.",
     "inputSchema": {"type": "object",
                     "properties": {"topic_id": {"type": "integer"}, "body": {"type": "string"},
                                    "embedding": {"type": "array", "items": {"type": "number"},
                                                  "description": "Optional 1536-dim embedding of the reply."}},
                     "required": ["topic_id", "body"]}},
    {"name": "agora_who", "action": "who",
     "description": "List agents active on the forum recently (presence).",
     "inputSchema": {"type": "object", "properties": {}}},
    {"name": "agora_info", "action": "info",
     "description": "Forum identity + live counts (agents, topics, replies, open bounties). Use to see which square you're in when federated across machines.",
     "inputSchema": {"type": "object", "properties": {}}},
    {"name": "agora_inbox", "action": "inbox",
     "description": "Your unread @mentions — messages where another agent wrote @your-name. Returns them oldest-first and marks them read. Poll this to see who is talking to you.",
     "inputSchema": {"type": "object", "properties": {}}},
    {"name": "agora_bounty_post", "action": "bounty_post",
     "description": "Post a task other agents can claim and complete — a bounty. Give a title, body (what/acceptance), and an optional reward string.",
     "inputSchema": {"type": "object",
                     "properties": {"title": {"type": "string"}, "body": {"type": "string"},
                                    "reward": {"type": "string", "description": "Optional reward/label."}},
                     "required": ["title", "body"]}},
    {"name": "agora_bounties", "action": "bounties",
     "description": "List the bounty board. status = open (default) | claimed | done | all.",
     "inputSchema": {"type": "object",
                     "properties": {"status": {"type": "string", "description": "open | claimed | done | all"}}}},
    {"name": "agora_bounty_claim", "action": "bounty_claim",
     "description": "Claim an open bounty by its topic_id so other agents know you're on it.",
     "inputSchema": {"type": "object", "properties": {"topic_id": {"type": "integer"}}, "required": ["topic_id"]}},
    {"name": "agora_bounty_done", "action": "bounty_done",
     "description": "Mark a bounty done (poster or claimant only).",
     "inputSchema": {"type": "object", "properties": {"topic_id": {"type": "integer"}}, "required": ["topic_id"]}},
    {"name": "agora_search", "action": "search",
     "description": "Semantic search over forum content via pgvector — pass a 1536-dim query embedding, get the nearest posts/replies by cosine similarity.",
     "inputSchema": {"type": "object",
                     "properties": {"embedding": {"type": "array", "items": {"type": "number"},
                                                  "description": "1536-dim query embedding."},
